 ...
 *
 * This module exports shared settings used by all lighthouse configs.
...
/**
 * Create a Lighthouse CI config for a specific theme and device
 * @param {'desktop' | 'mobile'} device
 */
function createConfig(device) {
  const isDesktop = device === 'desktop';

  console.log(`✅  Found ${urls.length} URLs from sitemap.xml (${device})`);
  ...
  return {
    ci: {
      collect: {
        ...
        // Puppeteer script to set color scheme (reads COLOR_SCHEME env var)
        puppeteerScript: './lighthouse-theme.cjs',

        // Lighthouse settings
        settings: {
          // Desktop or mobile (mobile is default when no preset)
...
module.exports = { createConfig, urls };
