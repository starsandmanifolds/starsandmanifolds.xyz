...
It polls an [AirGradient](https://www.airgradient.com/) sensor over the local network, decides whether the kitchen needs exhausting based on configurable thresholds for CO₂, PM2.5, TVOC, and NOx, and controls a [Tuya-compatible smart plug](https://www.aziot.in/product-page/10a-smart-plug-with-energy-monitoring) via its local encrypted protocol. The whole thing runs as a foreground service so it keeps going even when I close the app.
...
The exhaust fan is plugged into an [AZIOT 10A Smart Plug with Energy Monitoring](https://www.aziot.in/product-page/10a-smart-plug-with-energy-monitoring). It supports WiFi/Bluetooth, has a timer function, and works with Google Home and Alexa. Under the hood it runs Tuya's firmware and speaks Tuya's local protocol (version 3.5).
...
The app targets API 23+, uses no AndroidX or Compose — just plain Android Views and the Android SDK. The only library dependency is `kotlinx-coroutines-android` for the service loop. The Tuya crypto uses `javax.crypto` (AES/GCM/NoPadding, HmacSHA256) from the JDK.

Building requires:

1. An `autovent.local.properties` file (git-ignored) with the sensor URL, fan IP, device ID, and Tuya local key.
2. Android SDK 35.
3. Java 17+.
...
The release build is R8-minified and resource-shrunk. For local installation I sign with the debug keystore; for distribution you'd need a proper signing key.

![AutoVent main UI](/images/blog/autovent/autovent-main.png)

*The app UI showing four adjustable metric cards with band-style threshold sliders.*
...
- Better accessibility for the custom threshold slider.
- More resilient fan state tracking (verify the plug response after issuing a command).
- Possibly a small embedded sensor display that doesn't need a phone at all.
